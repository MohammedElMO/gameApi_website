...
const App = () => {
  ...
  return (
    <div className={"h-screen grid grid-areas-layout grid-rows-layout  "}>
      ...
      <section
        className={darkMode(
          "bg-[rgba(0,0,0,.9)]  grid-in-main max-md:col-start-nav ",
          darkmode,
          "bg-white"
        )}
      >
        ...
        <div className="flex">
          ...
          />
          <DropSort
            OnSortGames={(sortBy) => setGameQuery({ ...GameQuery, sortBy })}
          />
        </div>
      ...
      >
        <GenresDropList
          dark={darkmode}
          selectedGenres={GameQuery.genreId}
          onFilter={(genre) => setGameQuery({ ...GameQuery, genreId: genre })}
        />
      </section>
    </div>
  );
